...
def save_predictions_to_gcs(pred):
    ...
    file_path = f"/tmp/{today_date}.json"
...
def load_stage1_model():
    ...
    download_blob(bucket_name, 'stage1/column_transformer.joblib', '/tmp/column_transformer.joblib')
...
def load_stage2_model(model_index):
    ...
    download_blob(bucket_name, f'stage2/model_{model_index}.joblib', f'/tmp/model_{model_index}.joblib')
    download_blob(bucket_name, f'stage2/label_encoder_{model_index}.joblib', f'/tmp/label_encoder_{model_index}.joblib')
...
def transform_and_predict(model, df, label_encoder, column_transformer=None, pic=None):
    ...
    """
    X = df.drop('pic', axis=1, errors='ignore')  # Adjust 'pic' to the actual target column name if different
